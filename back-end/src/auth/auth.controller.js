...
async function sendEmail(email, subject, text) {
    ...
    try {
        await transporter.sendMail({
            from: '"Sistema de autenticação" <eletconnect@outlook.com>',
            to: email,
            subject,
            text
        });
...
exports.login = async (request, response) => {
    ...
    try {
        const { data: user, error: userERROR } = await supabase
            .from('usuarios')
            .select('id, nome, senha, status, avatar, confirmed_at')
            .eq('email', email)
            .single();
        ...
        request.session.user = { id: user.id, nome: user.nome, email, status: user.status, avatar: user.avatar };
